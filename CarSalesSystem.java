...
/**
 * This class is the main JFrame of the application, and deals with the car
 * collection.
 * It also creates instances of the other panels (add car, view car, etc) and
 * incorporates
 ...
 * public Car[] search(int minPrice, int maxPrice, double minDistance, double
 * maxDistance)
...
public class CarSalesSystem extends JFrame implements ActionListener, ComponentListener, ChangeListener {
	...
	/**
	 * is used as the parameter in the 'getStatistics(int type)' method to indicate
	 ...
	 * is used as the parameter in the 'getStatistics(int type)' method to indicate
	 * you wish to find the
	 * average distance travelled (in kilometers) from the entire collection of cars
	 * in the system
	 */
	...
	/**
	 * @param f existing binary file for storing/retrieving car dataThe data file
	 */
	public CarSalesSystem(String f) {
		super("Car Sales");
		...
		// empty cars.dat file, this error should be ignored
			...
			System.out.println(
					"The data file, 'cars.dat' is possibly corrupted. Please delete it and create a new empty data file named cars.dat");
			System.exit(0);
		} catch (Exception exp) {
			System.out.println(
					"There was an error loading 'cars.dat'. Try deleting and creating a new empty file named 'cars.dat'");
			System.exit(0);
		}
	 ...
	 */
	public void aboutMenuItemClicked() {
		// if it doesn't exist, create a new instance, otherwise display the current
		// reference
		if (aboutDlg == null)
	...
	/**
	 * handles closing events for the Car Sales System. Saves any updated data to a
	 * binary file
	 */
	public void closing() {
		boolean ok;
			...
			do {
				try {
					...
					int result = JOptionPane.showConfirmDialog(this,
							"The data file could not be written, possibly because you don't have access to this location.\nIf you chose No to retry you will lose all car data from this session.\n\nWould you like to reattempt saving the data file?",
							"Problem saving data", JOptionPane.YES_NO_OPTION);
	...
	/**
	 * receives events when JFrame is resized and ensures the application doesn't
	 * get resized
	...
	/**
	 * Converts a range string such as "50-100" or "20+" to a double array with
	 * minimum and maximum
	 * bounds
	 *
	 * @param s a string containing a valid range. "200", "100-200", "20+" are all
	 *          examples
	 *          of valid ranges
	 * @return array of 2 elements. First element indicating minimum bounds, second
	 *         element
	 *         indicating maximum bounds. A value of -1 for minimum and maximum
	 *         bounds indicates an error
	 *         in converting the parameter to a proper range. A value of -1 for the
	 *         maximum bounds and
	 *         a value > 0 for the minimum bounds indicates the maximum bounds is
	 *         infinity in theory.
	 *         If both bounds are greater than 0, this indicates a valid range a to
	 *         b, where a and b
	 *         can be equal.
	 */
	public static double[] convertToRange(String s) {
	 ...
	 *
	 * @param type can be either CARS_COUNT, MANUFACTURERS_COUNT, AVERAGE_PRICE,
	 *             AVERAGE_DISTANCE or
	 *             AVERAGE_AGE
	...
	/**
	 * call this method to alert the Car Sales System that a car has been added, and
	 * also send
	...
	public void setCarsUpdated() {
		...
		for (int i = 0; i < registeredListeners.size(); i++) {
			...
			try {
				...
				java.lang.reflect.Method callingMethod = registeredListeners.get(i).getClass().getMethod("carsUpdated",
						paramType);
				// invoke the method with our parameters
				callingMethod.invoke(registeredListeners.get(i), param);
			} catch (NoSuchMethodException exp) {
				System.out.println("Warning, 'public carsUpdated(CarEvent)' method does not exist in "
						+ registeredListeners.get(i).getClass().getName()
						+ ". You will not receive any car update events");
			} catch (IllegalAccessException exp) {
				System.out.println(
						"Warning, the 'public carUpdated(CarEvent)' method couldn't be called for unknown reasons, You will not receive any car update events");
			} catch (Exception exp) {
			}
		}
	}
	...
	/**
	 * the selected tab in the JTabbedPane has been changed, so change the title bar
	 * text
	 *
